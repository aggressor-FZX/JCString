...
// Section:  33616
// Program JCString V2
// Description: HEADER FILE  Declarations 
// Shows member functions of the JCString class 
...
public:
	...
	JCString returnLower() const;	// returns a new JCString copy but lowercase
private:
	char* str;						// the array it is based off called str;
	int end;						// the index of the last char
	int cap;						// the max memory to use
	int JCScompareTo(const JCString& argStr) const;
	...
	// STATIC FUNCTIONS
	static char* appendCstr(const  char* str1, const char* str2); // Appends two C strings
